...
//I want to replace this with a lighting profiles value later but not yet
//When we flatten the light radius multipliers these will be multiplied by 1.25f
#define DEFAULT_LASER_LIGHT_RADIUS 100.0f
#define DEFAULT_MISSILE_LIGHT_RADIUS 20.0f
...
typedef struct beam_weapon_info {
	...
	float beam_light_radius;			// Custom light radius
...
struct weapon_info
{
	...
	bool		has_light_radius;
	bool 		has_custom_light;
	hdr_color 	custom_light_color;		//For the light cast by the projectile
	float 		light_radius;
